...
workflow HUMAN_WGS
{
	...
	if (params.debug) {
		...
		PREPARE_GENOME.out.chrom_names.view {"chrom_names:\n${it}"}
		PREPARE_GENOME.out.interval_bed.view {"out.interval_bed:\n${it}"}
		GENOME_ANNOTATION.out.gc_wig.view {"out.gc_wig:\n${it}"}
		GENOME_ANNOTATION.out.map_wig.view {"out.map_wig:\n${it}"}
		ch_bam.view {"ch_bam:\n${it}"}
	}
