...
srun --ntasks=1 --exclusive --mem 45568mb -J $sample -o ./ASHPC_0019_Pa_P_5262.%j.%N.out ${mocaseqDir}/launch/ccc_remap_wrapper.sh -ccc $cccDir -wd $workingDir -m $mocaseqDir -bd $bamDir -bf $bamName -s $sample -rd $referencesDir -t $bamType -@ 32 > ${sample}-remap.out & 
sleep 4
...
bamType="Normal"
# submit subjob for sample remapping
srun --ntasks=1 --exclusive --mem 45568mb -J $sample -o ./ASHPC_0019_Pa_R.%j.%N.out ${mocaseqDir}/launch/ccc_remap_wrapper.sh -ccc $cccDir -wd $workingDir -m $mocaseqDir -bd $bamDir -bf $bamName -s $sample -rd $referencesDir -t $bamType -@ 32 > ${sample}-remap.out & 
sleep 4
