#!/bin/bash
#SBATCH -J batch04-ASHPC_0028-remap
#SBATCH -D ./
...
# SLURM script for remapping sample group batch04-ASHPC_0028 .bam files hg19 -> hg38.
...
# specify sample
sample=ASHPC_0028_Pa_P
bamDir=/dss/dssfs03/tumdss/pn72lo/pn72lo-dss-0006/projects/hPDAC/ICGC_PACA_CA_WGS/input/GRCh37_bam/batch04
bamName=EGAZ00001379804_PCSI_wgs_bam_ASHPC_0028_Pa_P.bam
bamType="Tumor"
# submit subjob for sample remapping
srun --ntasks=1 --exclusive --mem 45568mb -J $sample -o ./ASHPC_0028_Pa_P.%j.%N.out ${mocaseqDir}/launch/ccc_remap_wrapper.sh -ccc $cccDir -wd $workingDir -m $mocaseqDir -bd $bamDir -bf $bamName -s $sample -rd $referencesDir -t $bamType > ${sample}-remap.out & 
sleep 4

# specify sample
sample=ASHPC_0028_Pa_R
bamDir=/dss/dssfs03/tumdss/pn72lo/pn72lo-dss-0006/projects/hPDAC/ICGC_PACA_CA_WGS/input/GRCh37_bam/batch04
bamName=EGAZ00001379805_PCSI_wgs_bam_ASHPC_0028_Pa_R.bam
bamType="Normal"
# submit subjob for sample remapping
srun --ntasks=1 --exclusive --mem 45568mb -J $sample -o ./ASHPC_0028_Pa_R.%j.%N.out ${mocaseqDir}/launch/ccc_remap_wrapper.sh -ccc $cccDir -wd $workingDir -m $mocaseqDir -bd $bamDir -bf $bamName -s $sample -rd $referencesDir -t $bamType > ${sample}-remap.out & 
sleep 4
