...
def main():
    ...
    ofile.write("time python scripts/filter_variant_input.py -if={0} -of={1}\n".format(inputVariantFile, interimFileName))
    ...
    ofile.write("time python scripts/merge_annotation_files.py -if={0} -pf={1} -sf={2} -rf={3} -gb={4} -ab={5} -gr={6} -cp={7} -ti={8} -tx={9} -nx={10} -sn={11}\n".format(inputVariantFile, snvPileupFile, segmentalDupBedFile, repeatMaskerBedFile, snvBinGCpcFile, tenBpAroundSNVFile, snvReadMeanGCpcFile, closestProbeFile, tumorIndelFile, extParsedTumorPileupFile, extParsedNormalPileupFile, sampleName))
    ofile.close()
