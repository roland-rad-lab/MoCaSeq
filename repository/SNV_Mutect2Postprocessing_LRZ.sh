...
echo '---- Mutect2 Postprocessing I (OrientationFilter, Indel size selection, filtering) ----' 
echo "$(date) \t timestamp: $(date +%s)" 
...
if [ $artefact_type = 'no' ]; then
	...
	--variant ${name}/results/Mutect2/${name}.${type}.m2.vcf.gz \
...
echo '---- Mutect2 Postprocessing II (Filtering out known SNV/Indel using dbSNP or the Sanger Mouse database) ----' 
echo "$(date) \t timestamp: $(date +%s)" 
...
echo '---- Mutect2 Postprocessing III (Annotate calls) ----' 
echo "$(date) \t timestamp: $(date +%s)" 
