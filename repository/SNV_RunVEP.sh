...
if [ $input_format = 'Mutect2' ]; then
	...
	do
		...
		awk 'BEGIN{FS=OFS="\t"} FNR==1{print $0}' $name/results/$input_format/$name.$type.$input_format.vep.maf > $name/results/$input_format/$name.$type.$input_format.vep.tmp
		awk 'BEGIN{FS=OFS="\t"} FNR==2{print $0,$NF="t_maf"}' $name/results/$input_format/$name.$type.$input_format.vep.maf >> $name/results/$input_format/$name.$type.$input_format.vep.tmp
